...
Public Class frm_importar_excel_REGICONT

    '===================================
    '*** IMPORTAR REGISTRO Y CONTROL ***
    '===================================
...
#Region "INSTANCIAR FORMULARIO"
    ...
    Public Shared Function instance() As frm_importar_excel_REGICONT
        If frm_Instance Is Nothing OrElse frm_Instance.isdisposed = True Then
            frm_Instance = New frm_importar_excel_REGICONT
        End If
...
#Region "PROCEDIMIENTOS"
    ...
    End Sub
    Private Sub pro_ValidarRegistroyControl()
    ...
    End Sub
    Private Sub pro_GuardarRegistroyControl()
        ...
        Try
            ...
            Dim inRECONUFI As Integer = 0
            Dim stRECOINGE As String = ""
            Dim stRECOPRED As String = ""
            Dim stRECOEMPR As String = ""
            Dim stRECOPROP As String = ""
            Dim stRECOREPR As String = ""
            Dim stRECORECH As String = ""

            For i = 0 To dt_JURILIST.Rows.Count - 1

                ' almacena los datos
                inRECONUFI = CInt(dt_JURILIST.Rows(i).Item(0).ToString)
                stRECOINGE = CType(Trim(dt_JURILIST.Rows(i).Item(1).ToString), String)
                stRECOPRED = CType(Trim(dt_JURILIST.Rows(i).Item(2).ToString), String)
                stRECOEMPR = CType(Trim(dt_JURILIST.Rows(i).Item(3).ToString), String)
                stRECOPROP = CType(Trim(dt_JURILIST.Rows(i).Item(4).ToString), String)
                stRECOREPR = CType(Trim(dt_JURILIST.Rows(i).Item(5).ToString), String)
                stRECORECH = CType(Trim(dt_JURILIST.Rows(i).Item(6).ToString), String)

                ' instancia la clase
                Dim obFICHPRED As New cla_FICHPRED
                Dim dtFICHPRED As New DataTable

                dtFICHPRED = obFICHPRED.fun_Consultar_FICHA_PREDIAL_X_NRO_FICHA_PREDIAL(inRECONUFI)

                If dtFICHPRED.Rows.Count > 0 Then

                    ' instancio la clase
                    Dim oJuridica As New cla_registro_y_control_FICHPRED
                    Dim dtJuridica As New DataTable

                    dtJuridica = oJuridica.fun_Buscar_NRO_FICHA_REGICONT(inRECONUFI)

                    If dtJuridica.Rows.Count > 0 Then

                        ' elimina el registro
                        oJuridica.fun_Eliminar_NRO_FICHA_REGICONT(inRECONUFI)

                        ' inserta el registro
                        oJuridica.fun_Insertar_REGICONT(inRECONUFI, stRECOINGE, stRECOPRED, stRECOEMPR, stRECOPROP, stRECOREPR, stRECORECH)
                    Else
                        ' inserta el registro
