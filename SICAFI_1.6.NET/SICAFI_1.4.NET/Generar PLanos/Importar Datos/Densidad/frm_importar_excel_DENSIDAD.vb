...
Public Class frm_importar_excel_DENSIDAD

    '==========================================
    '*** FORMULARIO IMPORTAR EXCEL DENSIDAD ***
    '==========================================
...
#Region "INSTANCIAR FORMULARIO"
    ...
    Public Shared Function instance() As frm_importar_excel_DENSIDAD
        If frm_Instance Is Nothing OrElse frm_Instance.isdisposed = True Then
            frm_Instance = New frm_importar_excel_DENSIDAD
        End If
...
#Region "PROCEDIMIENTOS"
    ...
    Private Sub pro_ValidarCategoriaFichaPredial()
        ...
        ' Crea las columnas
        dt1.Columns.Add(New DataColumn("CedulaCatastral", GetType(String)))
        dt1.Columns.Add(New DataColumn("Densidad", GetType(String)))
        ...
        For i = 0 To dt_FPCELIST.Rows.Count - 1
            ...
            ' verifica que el campo cedula catastral
            If Trim(dt_FPCELIST.Rows(i).Item(0).ToString) = "" Then

                dr1 = dt1.NewRow()

                dr1("CedulaCatastral") = Trim(dt_FPCELIST.Rows(i).Item(0).ToString)
                dr1("Densidad") = Trim(dt_FPCELIST.Rows(i).Item(1).ToString)
                dr1("Descripcion") = "El campo nro de cedula catastral esta vacio"

                dt1.Rows.Add(dr1)

            Else

                If Trim(dt_FPCELIST.Rows(i).Item(0).ToString).Length <> 19 Then

                    dr1 = dt1.NewRow()

                    dr1("CedulaCatastral") = Trim(dt_FPCELIST.Rows(i).Item(0).ToString)
                    dr1("Densidad") = Trim(dt_FPCELIST.Rows(i).Item(1).ToString)
                    dr1("Descripcion") = "El campo nro de cedula catastral longitud <> 19 caracteres"
            ...
            ' verifica que el campo densidad
            If Trim(dt_FPCELIST.Rows(i).Item(1).ToString) = "" Then
                ...
                dr1("CedulaCatastral") = Trim(dt_FPCELIST.Rows(i).Item(0).ToString)
                dr1("Densidad") = Trim(dt_FPCELIST.Rows(i).Item(1).ToString)
                dr1("Descripcion") = "El campo densidad esta vacio"

                dt1.Rows.Add(dr1)

            Else

                If fun_Verificar_Dato_Decimal_Evento_Validated(Trim(dt_FPCELIST.Rows(i).Item(1).ToString)) = False Then

                    dr1 = dt1.NewRow()

                    dr1("CedulaCatastral") = Trim(dt_FPCELIST.Rows(i).Item(0).ToString)
                    dr1("Densidad") = Trim(dt_FPCELIST.Rows(i).Item(1).ToString)
                    dr1("Descripcion") = "El campo densidad no es numerico"
    ...
    Private Sub pro_GuardarCategoriaFichaPredial()
        ...
        Try
            ...
            For i = 0 To dt_FPCELIST.Rows.Count - 1
                ...
                stFIPRCECA = Trim(dt_FPCELIST.Rows(i).Item(0).ToString)
                stFIPRDENS = Trim(dt_FPCELIST.Rows(i).Item(1).ToString)
                stFIPRESTA = "ac"

                Dim boFIPRCECA As Boolean = fun_Verificar_Dato_Numerico_Evento_Validated(stFIPRCECA)
                Dim boFIPRDENS As Boolean = fun_Verificar_Dato_Decimal_Evento_Validated(stFIPRDENS)

                If boFIPRCECA = True And boFIPRDENS = True Then

                    ' Instancia la clase
                    Dim objdatos As New cla_DENSIDAD
                    Dim tbl As New DataTable

                    tbl = objdatos.fun_Buscar_CODIGO_DENSIDAD(Trim(stFIPRCECA))

                    If tbl.Rows.Count > 0 Then

                        ' elimina el registro
                        objdatos.fun_Eliminar_x_CECA_MANT_DENSIDAD(Trim(stFIPRCECA))

                        ' inserta el registro
                        objdatos.fun_Insertar_MANT_DENSIDAD(Trim(stFIPRCECA), Trim(stFIPRDENS), Trim(stFIPRESTA))
