...
Public Class frm_importar_excel_JURIDICA

#Region "INSTANCIAR FORMULARIO"

    Private Shared frm_Instance = Nothing

    Public Shared Function instance() As frm_importar_excel_JURIDICA
        If frm_Instance Is Nothing OrElse frm_Instance.isdisposed = True Then
            frm_Instance = New frm_importar_excel_JURIDICA
        End If
...
#Region "VARIABLES"
    ...
    Dim dt_JURILIST As New DataTable
...
#Region "PROCEDIMIENTOS"
    ...
    End Sub
    Private Sub pro_ValidarInvestigacionJuridica()
        ...
        ' crea la tabla
        dt_JURILIST = New DataTable

        ' carga la tabla
        dt_JURILIST = Me.dgvFIPRLIST.DataSource

        For i = 0 To dt_JURILIST.Rows.Count - 1

            ' verifica el 
            If Trim(dt_JURILIST.Rows(i).Item(0).ToString) = "" Then
            ...
            Else
                ...
                If fun_Verificar_Dato_Numerico_Evento_Validated(Trim(dt_JURILIST.Rows(i).Item(0).ToString)) = False Then
            ...
            ' verifica el campo 
            If Trim(dt_JURILIST.Rows(i).Item(7).ToString) = "" Then

                dr1 = dt1.NewRow()

                dr1("Nro_Ficha") = Trim(dt_JURILIST.Rows(i).Item(0).ToString)
                dr1("Inconsistencia") = "El campo falsa tradicion no esta diligenciado como VERDADERO o FALSO"
            ...
            ' verifica el campo 
            If Trim(dt_JURILIST.Rows(i).Item(9).ToString) = "" Then

                dr1 = dt1.NewRow()

                dr1("Nro_Ficha") = Trim(dt_JURILIST.Rows(i).Item(0).ToString)
                dr1("Inconsistencia") = "El campo plano protocolizado no esta diligenciado como VERDADERO o FALSO"
    ...
    End Sub
    Private Sub pro_GuardarAreaTerrenoFichaPredial()
        ...
        Try
            ...
            Dim inJURINUFI As Integer = 0
            Dim stJURIFOLI As String = ""
            Dim stJURIMASE As String = ""
            Dim stJURIMAAB As String = ""
            Dim stJURITOAB As String = ""
            Dim stJURIFOAB As String = ""
            Dim stJURIARTE As String = ""
            Dim boJURIFATR As Boolean = False
            Dim stJURIADEO As String = ""
            Dim boJURIPLPR As Boolean = False
            Dim stJURIOBSE As String = ""

            For i = 0 To dt_JURILIST.Rows.Count - 1

                ' almacena los datos
                inJURINUFI = CInt(dt_JURILIST.Rows(i).Item(0).ToString)
                stJURIFOLI = CType(Trim(dt_JURILIST.Rows(i).Item(1).ToString), String)
                stJURIMASE = CType(Trim(dt_JURILIST.Rows(i).Item(2).ToString), String)
                stJURIMAAB = CType(Trim(dt_JURILIST.Rows(i).Item(3).ToString), String)
                stJURITOAB = CType(Trim(dt_JURILIST.Rows(i).Item(4).ToString), String)
                stJURIFOAB = CType(Trim(dt_JURILIST.Rows(i).Item(5).ToString), String)
                stJURIARTE = CType(Trim(dt_JURILIST.Rows(i).Item(6).ToString), String)
                boJURIFATR = CBool(dt_JURILIST.Rows(i).Item(7).ToString)
                stJURIADEO = CType(Trim(dt_JURILIST.Rows(i).Item(8).ToString), String)
                boJURIPLPR = CBool(dt_JURILIST.Rows(i).Item(9).ToString)
                stJURIOBSE = CType(Trim(dt_JURILIST.Rows(i).Item(10).ToString), String)

                ' instancio la clase
                Dim oJuridica As New cla_juridica_PREDIO
                Dim dtJuridica As New DataTable

                dtJuridica = oJuridica.fun_Buscar_NRO_FICHA_JURIDICA(inJURINUFI)

                If dtJuridica.Rows.Count > 0 Then

                    ' elimina el registro
                    oJuridica.fun_Eliminar_NRO_FICHA_JURIDICA(inJURINUFI)

                    ' inserta el registro
                    oJuridica.fun_Insertar_JURIDICA(inJURINUFI, stJURIFOLI, stJURIMASE, stJURIMAAB, stJURITOAB, stJURIFOAB, stJURIARTE, boJURIFATR, stJURIADEO, boJURIPLPR, stJURIOBSE)
                Else
                    ' inserta el registro
    ...
    Private Sub cmdValidaDatos_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdValidaDatos.Click
        Try
            ...
            If Trim(stRutaArchivo) <> "" And Me.dgvFIPRLIST.RowCount > 0 Then
                ...
                pro_ValidarInvestigacionJuridica()
        ...
        Try
            If Me.dgvFIPRINCO.RowCount = 0 Then
                If Trim(stRutaArchivo) <> "" Then
                    ...
                    pro_GuardarAreaTerrenoFichaPredial()
