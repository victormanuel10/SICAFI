...
Public Class frm_modificar_FORMMUNI

    '===================================
    '*** MODIFICAR FORMULA MUNICIPIO ***
    '===================================
...
#Region "PROCEDIMIENTOS"
    ...
    Private Sub pro_inicializarControles()
        ...
        Try
            ...
            Me.cboFOMUDEPA.DataSource = objdatos1.fun_Consultar_CAMPOS_MANT_DEPARTAMENTO
            Me.cboFOMUDEPA.DisplayMember = "DEPADESC"
            Me.cboFOMUDEPA.ValueMember = "DEPACODI"

            Dim objdatos2 As New cla_CLASSECT

            Me.cboFOMUCLSE.DataSource = objdatos2.fun_Consultar_CAMPOS_MANT_CLASSECT
            Me.cboFOMUCLSE.DisplayMember = "CLSEDESC"
            Me.cboFOMUCLSE.ValueMember = "CLSECODI"

            Dim objdatos27 As New cla_VIGENCIA

            Me.cboFOMUVIGE.DataSource = objdatos27.fun_Consultar_CAMPOS_VIGENCIA
            Me.cboFOMUVIGE.DisplayMember = "VIGEDESC"
            Me.cboFOMUVIGE.ValueMember = "VIGECODI"

            Dim objdatos50 As New cla_TIPOIMPU

            Me.cboFOMUTIIM.DataSource = objdatos50.fun_Consultar_CAMPOS_MANT_TIPOIMPU
            Me.cboFOMUTIIM.DisplayMember = "TIIMDESC"
            Me.cboFOMUTIIM.ValueMember = "TIIMCODI"

            Dim objdatos28 As New cla_FORMULA

            Me.cboFOMUFORM.DataSource = objdatos28.fun_Consultar_CAMPOS_FORMULA
            Me.cboFOMUFORM.DisplayMember = "FORMDESC"
            Me.cboFOMUFORM.ValueMember = "FORMCODI"

            Dim objdatos As New cla_ESTADO

            Me.cboFOMUESTA.DataSource = objdatos.fun_Consultar_TODOS_LOS_ESTADOS
            Me.cboFOMUESTA.DisplayMember = "ESTADESC"
            Me.cboFOMUESTA.ValueMember = "ESTACODI"

            Dim objdatos3 As New cla_FORMMUNI
            Dim tbl As New DataTable

            tbl = objdatos3.fun_Buscar_ID_FORMMUNI(inID_REGISTRO)

            Me.cboFOMUDEPA.SelectedValue = tbl.Rows(0).Item("FOMUDEPA")

            Dim objdatos7 As New cla_MUNICIPIO

            Me.cboFOMUMUNI.DataSource = objdatos7.fun_Consultar_CAMPOS_MANT_MUNICIPIO
            Me.cboFOMUMUNI.DisplayMember = "MUNIDESC"
            Me.cboFOMUMUNI.ValueMember = "MUNICODI"

            Me.cboFOMUMUNI.SelectedValue = tbl.Rows(0).Item("FOMUMUNI")
            Me.cboFOMUCLSE.SelectedValue = tbl.Rows(0).Item("FOMUCLSE")
            Me.cboFOMUVIGE.SelectedValue = tbl.Rows(0).Item("FOMUVIGE")
            Me.cboFOMUTIIM.SelectedValue = tbl.Rows(0).Item("FOMUTIIM")

            Dim objdatos51 As New cla_CONCEPTO

            Me.cboFOMUCONC.DataSource = objdatos51.fun_Consultar_CAMPOS_MANT_CONCEPTO
            Me.cboFOMUCONC.DisplayMember = "CONCDESC"
            Me.cboFOMUCONC.ValueMember = "CONCCODI"

            Me.cboFOMUCONC.SelectedValue = tbl.Rows(0).Item("FOMUCONC")
            Me.cboFOMUFORM.SelectedValue = tbl.Rows(0).Item("FOMUFORM")
            Me.txtFOMUDESC.Text = Trim(tbl.Rows(0).Item("FOMUDESC"))
            Me.chkFOMUIMPR.Checked = tbl.Rows(0).Item("FOMUIMPR")
            Me.chkFOMUACBO.Checked = tbl.Rows(0).Item("FOMUACBO")
            Me.chkFOMUALPU.Checked = tbl.Rows(0).Item("FOMUALPU")
            Me.chkFOMUTAAS.Checked = tbl.Rows(0).Item("FOMUTAAS")
            Me.cboFOMUESTA.SelectedValue = tbl.Rows(0).Item("FOMUESTA")

            Dim boFOMUDEPA As Boolean = fun_Buscar_Dato_DEPARTAMENTO(Me.cboFOMUDEPA.SelectedValue)
            Dim boFOMUMUNI As Boolean = fun_Buscar_Dato_MUNICIPIO(Me.cboFOMUDEPA.SelectedValue, Me.cboFOMUMUNI.SelectedValue)
            Dim boFOMUCLSE As Boolean = fun_Buscar_Dato_CLASSECT(Me.cboFOMUCLSE.SelectedValue)
            Dim boFOMUVIGE As Boolean = fun_Buscar_Dato_VIGENCIA(Me.cboFOMUVIGE.SelectedValue)
            Dim boFOMUTIIM As Boolean = fun_Buscar_Dato_TIPOIMPU(Me.cboFOMUTIIM.SelectedValue)
            Dim boFOMUCONC As Boolean = fun_Buscar_Dato_CONCEPTO(Me.cboFOMUTIIM.SelectedValue, Me.cboFOMUCONC.SelectedValue)
            Dim boFOMUFORM As Boolean = fun_Buscar_Dato_FORMULA(Me.cboFOMUFORM.SelectedValue)

            If boFOMUDEPA = True OrElse _
               boFOMUMUNI = True OrElse _
               boFOMUCLSE = True OrElse _
               boFOMUVIGE = True OrElse _
               boFOMUTIIM = True OrElse _
               boFOMUCONC = True OrElse _
               boFOMUFORM = True Then
        ...
        Try
            ...
            If boFOMUDEPA = True And _
               ...
               boFOMUESTA = True Then
                ...
                If (objdatos22.fun_Actualizar_FORMMUNI(inID_REGISTRO, _
                                                         Me.cboFOMUDEPA.SelectedValue, _
                                                         ...
                                                         Me.cboFOMUESTA.SelectedValue)) = True Then
...
#Region "KeyDown"
    ...
    Private Sub cboFOMUCONC_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cboFOMUCONC.KeyDown
        If e.KeyCode = Keys.Up Or e.KeyCode = Keys.Down Then
            fun_Cargar_ComboBox_Con_Datos_Activos_CONCEPTO_Descripcion(Me.cboFOMUCONC, Me.cboFOMUCONC.SelectedIndex, Me.cboFOMUDEPA)
        End If
    End Sub
