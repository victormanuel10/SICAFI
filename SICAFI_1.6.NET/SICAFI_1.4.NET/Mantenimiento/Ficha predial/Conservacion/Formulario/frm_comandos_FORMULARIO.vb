...
Public Class frm_comandos_FORMULARIO
...
#Region "PROCEDIMIENTOS"
    ...
    Private Sub pro_InicializarControlesModificar()
        ...
        Try
            ...
            tbl = objdatos1.fun_Buscar_ID_MANT_FORMULARIO(inIDREGISTRO)

            Me.txtFORMCODI.Text = CStr(Trim(tbl.Rows(0).Item("FORMCODI").ToString))
            Me.txtFORMDESC.Text = CStr(Trim(tbl.Rows(0).Item("FORMDESC").ToString))
            Me.cboFORMESTA.SelectedValue = tbl.Rows(0).Item("FORMESTA").ToString

            Me.txtFORMCODI.ReadOnly = True
            Me.ClientSize = New System.Drawing.Size(619, 230)
            Me.MaximumSize = New System.Drawing.Size(619, 230)
            Me.MinimumSize = New System.Drawing.Size(619, 230)
            Me.fraCOMANDOS2.Visible = False
    ...
    Private Sub pro_LimpiarCampos()
        ...
        pp_pro_LimpiarCampos(fraFORMULARIO)
    ...
    Private Sub pro_Consultar()
        ...
        Try
            ...
            Dim stFORMCODI As String = ""
            Dim stFORMDESC As String = ""
            Dim stFORMESTA As String = ""

            ' carga los campos
            If Trim(Me.txtFORMCODI.Text) = "" Then
                stFORMCODI = "%"
            Else
                stFORMCODI = Trim(Me.txtFORMCODI.Text)
            End If

            ' carga los campos
            If Trim(Me.txtFORMDESC.Text) = "" Then
                stFORMDESC = "%"
            Else
                stFORMDESC = Trim(Me.txtFORMDESC.Text)
            End If

            If Trim(Me.cboFORMESTA.SelectedValue) = "" Then
                stFORMESTA = "%"
            Else
                stFORMESTA = Trim(Me.cboFORMESTA.SelectedValue)
            End If

            'TIIMatena la consulta
            stConsulta += "Select "
            stConsulta += "FORMIDRE , "
            stConsulta += "FORMCODI as 'Codigo', "
            stConsulta += "FORMDESC as 'Descripción', "
            stConsulta += "FORMESTA as 'Estado' "
            stConsulta += "From MANT_FORMULARIO "
            stConsulta += "Where "
            stConsulta += "FORMCODI like '" & stFORMCODI & "' and "
            stConsulta += "FORMDESC like '" & stFORMDESC & "' and "
            stConsulta += "FORMESTA like '" & stFORMESTA & "' "
            stConsulta += "Order by FORMCODI"
    ...
    Private Sub pro_Insertar()
        ...
        Try
            ...
            Dim boLLAVEPRIMARIA As Boolean = objdatos.fun_Verifica_llave_Primaria_MANT_FORMULARIO(Me.txtFORMCODI)

            Dim objVerifica As New cla_Verificar_Dato

            Dim boCLASCODI As Boolean = objVerifica.fun_Verificar_Dato_Lleno(Me.txtFORMCODI)
            Dim boCLASDESC As Boolean = objVerifica.fun_Verificar_Dato_Lleno(Me.txtFORMDESC)
            Dim boCLASESTA As Boolean = objVerifica.fun_Verificar_Dato_Lleno(Me.cboFORMESTA)

            ' verifica los datos del formulario 
            If boLLAVEPRIMARIA = True And boCLASCODI = True And boCLASDESC = True And boCLASESTA = True Then

                Dim objdatos22 As New cla_FORMULARIO

                If (objdatos22.fun_Insertar_MANT_FORMULARIO(Me.txtFORMCODI.Text, Me.txtFORMDESC.Text, Me.cboFORMESTA.SelectedValue)) Then
    ...
    Private Sub pro_Modificar()
        ...
        Try
            ...
            Dim boTITRCODI As Boolean = objVerifica.fun_Verificar_Dato_Lleno(Me.txtFORMCODI)
            Dim boTITRDESC As Boolean = objVerifica.fun_Verificar_Dato_Lleno(Me.txtFORMDESC)
            Dim boTITRESTA As Boolean = objVerifica.fun_Verificar_Dato_Lleno(Me.cboFORMESTA)
            ...
            If boTITRCODI = True And boTITRDESC = True And boTITRESTA = True Then
                ...
                If objdatos.fun_Actualizar_MANT_FORMULARIO(inIDREGISTRO, Me.txtFORMCODI.Text, Me.txtFORMDESC.Text, Me.cboFORMESTA.SelectedValue) = True Then
                    Me.txtFORMDESC.Focus()
                    Me.Close()
...
#Region "MENU"
    ...
    Private Sub cmdACEPTAR_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdACEPTAR.Click
        ...
        Else
            Dim inId_Reg As New frm_FORMULARIO(Integer.Parse(Me.dgvCONSULTA.SelectedRows.Item(0).Cells(0).Value.ToString()))
            Me.txtFORMCODI.Focus()
            Me.Close()
        End If
...
#Region "KeyDown"
    ...
    Private Sub cboCLASESTA_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cboFORMESTA.KeyDown
