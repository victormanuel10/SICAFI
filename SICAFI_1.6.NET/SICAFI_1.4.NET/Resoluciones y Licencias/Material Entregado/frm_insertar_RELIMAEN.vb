...
Public Class frm_insertar_RELIMAEN

    '===================================================
    '*** MATERIAL ENTREGADO RESOLUCIONES Y LICENCIAS ***
    '===================================================
...
#Region "PROCEDIMIENTOS"
    ...
    Private Sub pro_inicializarControles()
        ...
        Try
            ...
            Dim objdatos9 As New cla_RELIMAEN
            Dim tbl9 As New DataTable

            tbl9 = objdatos9.fun_Buscar_ID_X_CONSULTA_PARAMETRIZADA_RELIMAEN(inRLPRSECU)

            If tbl9.Rows.Count > 0 Then

                Me.txtRLMEFECH.Text = Trim(tbl9.Rows(0).Item("RLMEFECH"))
                'Me.txtRLMEOBSE.Text = Trim(tbl9.Rows(0).Item("RLMEOBSE").ToString)

                If fun_Verificar_Dato_Fecha_Evento_Validated(Me.txtRLMEFECH.Text) = True Then
                    Me.DateTimePicker1.Value = CDate(tbl9.Rows(0).Item("RLMEFECH"))
                End If
                ...
                For i = 0 To tbl9.Rows.Count - 1
                    ...
                    dr1("Código") = Trim(tbl9.Rows(i).Item("RLMEMACA").ToString)
...
#Region "MENU"
    ...
    Private Sub cmdGUARDAR_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdGUARDAR.Click
        ...
        Try
            ...
            Dim obRECLOBME As New cla_RELIOBME
            Dim dtRECLOBME As New DataTable

            dtRECLOBME = obRECLOBME.fun_Buscar_CODIGO_X_RELIOBME(inRLPRSECU)

            If dtRECLOBME.Rows.Count = 0 Then
                obRECLOBME.fun_Insertar_RELIOBME(inRLPRSECU, inRLPRNURA, stRLPRFERA, " Nota ingresada por: " & vp_usuario & " " & fun_fecha() & " : " & Trim(Me.txtRLMEOBSE.Text) & ". ")
            Else

                Dim stMAUSOBSE_NUEVA As String = Trim(Me.txtRLMEOBSE.Text)
                Dim stMAUSOBSE_ANTERIOR As String = Trim(dtRECLOBME.Rows(0).Item("ROMEOBSE"))

                If Trim(stMAUSOBSE_NUEVA) <> "" Then
                    stMAUSOBSE_ANTERIOR += vbCrLf & " Nota ingresada por: " & vp_usuario & " " & fun_fecha() & " : " & stMAUSOBSE_NUEVA & ". "
                End If

                obRECLOBME.fun_Actualizar_RELIOBME(inRLPRSECU, Trim(stMAUSOBSE_ANTERIOR))
            End If

            If Me.dgvMATEFALT.RowCount = 0 Then
                Dim objdatos15 As New cla_RELIMAEN
                objdatos15.fun_Eliminar_SECU_RELIMAEN(inRLPRSECU)
            ...
            Else
                ...
                Dim objdatos11 As New cla_RELIMAEN
                objdatos11.fun_Eliminar_SECU_RELIMAEN(inRLPRSECU)

                Dim tbl As New DataTable
                tbl = Me.dgvMATEFALT.DataSource

                Dim obj_RELIMAFA As New cla_RELIMAFA
                Dim dt_RELIMAFA As New DataTable

                dt_RELIMAFA = obj_RELIMAFA.fun_Buscar_ID_X_CONSULTA_PARAMETRIZADA_RELIMAFA(inRLPRSECU)
                ...
                For i = 0 To tbl.Rows.Count - 1
                    ...
                    While j1 < dt_RELIMAFA.Rows.Count And sw1 = 0
                        If tbl.Rows(i).Item("Código") = dt_RELIMAFA.Rows(j1).Item("RLMFMACA") Then
                            sw1 = 1

                            Dim objEliminaMaterialFaltante As New cla_RELIMAFA
                            objEliminaMaterialFaltante.fun_Eliminar_SECU_X_MACA_RELIMAFA(inRLPRSECU, dt_RELIMAFA.Rows(j1).Item("RLMFMACA"))
                        Else
                            j1 = j1 + 1
                        End If
                    End While

                    Dim objdatos12 As New cla_RELIMAEN
                    objdatos12.fun_Insertar_RELIMAEN(inRLPRSECU, inRLPRNURA, stRLPRFERA, stCODIGO, Me.txtRLMEFECH.Text, Me.txtRLMEOBSE.Text)
...
#Region "KeyPress"
    ...
    Private Sub txt_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtRLMEOBSE.KeyPress
...
#Region "GotFocus"
    ...
    Private Sub txt_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtRLMEOBSE.GotFocus, txtRLMEFECH.GotFocus
...
#Region "ValueChanged"
    ...
    Private Sub DateTimePicker1_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DateTimePicker1.ValueChanged
        Me.txtRLMEFECH.Text = Me.DateTimePicker1.Value
    End Sub
